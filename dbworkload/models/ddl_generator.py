...
def generate_ddls(zip_content_location, db_name, output_file_location, cluster_url, output_file_name, anonymize):
    """
    ...
    for original_table_name, table_schema in schema_dict.items():
        ...
        for pk in table_schema.primary_keys:
            if pk in mapping['columns'][original_table_name]:
                anonymized_primary_keys.append(mapping['columns'][original_table_name][pk])
...
def anonymize_create_table(create_statement, mapping, constraint_mapping=None, index_mapping=None):
    """
    ...
    table_pattern = re.compile(r'CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?([^\s(]+)', re.IGNORECASE)
    table_match = table_pattern.search(create_statement)

    if not table_match:
        return "Error: Could not find table name in CREATE TABLE statement.", constraint_mapping, index_mapping
    ...
    if original_table not in mapping['tables']:
        ...
        for mapped_table in mapping['tables'].keys():
            if mapped_table.endswith('.' + original_table) or original_table.endswith('.' + mapped_table):
                original_table = mapped_table
                found = True
                break

        if not found:
            return f"Error: Table '{original_table}' not found in the mapping.", constraint_mapping, index_mapping

    # Get the anonymized table name
    anonymized_table = mapping['tables'][original_table]

    # Replace the table name in the statement
    anonymized_statement = table_pattern.sub(f'CREATE TABLE IF NOT EXISTS {anonymized_table}', create_statement)
    ...
    try:
        ...
        if close_paren_index == -1:
            return "Error: Could not find matching parentheses in CREATE TABLE statement.", constraint_mapping, index_mapping
        ...
        if original_table in mapping['columns']:
            ...
            for line in lines:
                ...
                constraint_match = re.match(r'^\s*CONSTRAINT\s+([^\s]+)', line, re.IGNORECASE)
                ...
                if constraint_match:
                    ...
                    if constraint_name not in constraint_mapping['names']:
                        if is_pk:
                            ...
                            anonymized_constraint = f"p{constraint_mapping['counter']}_pkey"
                        else:
                            # For other constraints, use "c{num}" format
                            anonymized_constraint = f"c{constraint_mapping['counter']}"

                        constraint_mapping['names'][constraint_name] = anonymized_constraint
                        constraint_mapping['counter'] += 1

                    # Replace the constraint name
                    line = re.sub(r'(CONSTRAINT\s+)' + re.escape(constraint_name),
                    ...
                    for col_name, anon_col in column_mapping.items():
                        ...
                        line = re.sub(r'\b' + re.escape(col_name) + r'\b', anon_col, line)
                    ...
                    line = re.sub(r'(INDEX\s+)' + re.escape(index_name),
                ...
                elif re.match(r'^\s*(PRIMARY\s+KEY|FOREIGN\s+KEY|UNIQUE|CHECK)', line, re.IGNORECASE):
        ...
        return f"Error processing CREATE TABLE statement: {str(e)}", constraint_mapping, index_mapping
